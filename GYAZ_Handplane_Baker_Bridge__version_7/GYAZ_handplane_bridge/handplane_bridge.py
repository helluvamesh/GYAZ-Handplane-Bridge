...
class Op_GYAZ_HandplaneBridge_SavePreset (bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if preset_name != '' and preset_name != ' ':
            ...
            preset_names = set( map( lambda preset: preset.name, prefs.presets) )
...
class Op_GYAZ_HandplaneBridge_ExportToHandPlane (bpy.types.Operator):
    ...
    bl_description = "Go to HP: Export to Handplane, Bake with HP: Export to Handplane and start baking"
    bl_options = {'REGISTER', 'UNDO'}
                    ...
                    if len (active_pgroups) == 0:
                        ...
                        if len(groups_with_no_high_poly_item)>0 or len(groups_with_no_low_poly_item)>0 or len(groups_with_unset_objects)>0 or len(groups_with_missing_objects)>0:
                            ...
                            if good_to_go == False:
                                ...
                                hp_no_vert_color_warning = 'no vertex color in: '
...
class UI_GYAZ_HandplaneBridge (Panel):
    ...
    def draw (self, context):
        ...
        if scene.gyaz_hpb.menu == 'GROUPS':
            ...
            lay.label ('Global Settings:')
            lay.prop (scene.gyaz_hpb.global_settings, 'threadCount')    
            lay.prop (scene.gyaz_hpb.global_settings, 'backRayOffsetScale')    
            lay.prop (scene.gyaz_hpb.global_settings, 'downsampleInGeneratorSpace')    
            lay.prop (scene.gyaz_hpb.global_settings, 'buildSmoothedNormalsForHighRes')    
            lay.prop (scene.gyaz_hpb.global_settings, 'suppressTriangulationWarning')    

                       
        elif scene.gyaz_hpb.menu == 'EXPORT':
            
            lay.label ('Destination:')
            row = lay.row ().prop (scene.gyaz_hpb, 'output_folder_mode', expand=True)
            ...
            col.scale_y = 2       
            col.operator (Op_GYAZ_HandplaneBridge_ExportToHandPlane.bl_idname, text='GO TO HANDPLANE', icon_value=custom_icons['handplane'].icon_id).mode='GO_TO'
            col.operator (Op_GYAZ_HandplaneBridge_ExportToHandPlane.bl_idname, text='BAKE WITH HANDPLANE', icon_value=custom_icons['handplane'].icon_id).mode='BAKE'
            row = lay.row (align=True)
